...
// Firebase 설정 (서버 스크립트와 동일한 설정값 사용)
...
// 로컬 스토리지 캐시 키 정의
const CACHE_KEY = 'gwanakbap_cached_data';

// 공통 데이터 로드 함수 (캐시 우선 로드 + 백그라운드 동기화 SWR 패턴 적용)
async function fetchMealDataFromFirebase() {
  const dbRef = ref(db, 'gwanakbap');
  
  try {
    // 1. 최신 데이터를 Firebase에서 가져오기 시도
    const snapshot = await get(dbRef);
    
    if (snapshot.exists()) {
      const freshData = snapshot.val();
      // 성공 시 로컬 스토리지에 최신 데이터 캐싱
      localStorage.setItem(CACHE_KEY, JSON.stringify(freshData));
      return freshData;
    } else {
      throw new Error('Firebase에 식단 데이터가 존재하지 않습니다.');
    }
  } catch (err) {
    console.warn('네트워크 연결 불안정 또는 Firebase 조회 실패. 캐시 데이터를 로드합니다.', err);
    
    // 2. 실패 시 로컬에 저장된 캐시 데이터 반환 (로딩 대기 방지)
...
class Infinite2DRenderer {
  ...
  async init() {
    try {
      // 💡 1순위: 로컬 캐시 데이터가 있다면 즉시 화면에 먼저 그려서 로딩 제로화
      const cachedData = localStorage.getItem(CACHE_KEY);
      ...
      // 2순위: Firebase에서 최신 데이터 조회 (백그라운드 동기화)
      const data = await fetchMealDataFromFirebase();
